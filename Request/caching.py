...
if os.path.isfile("data.json"):
    with open("data.json","r") as saral_data:
...
serial_number=1
name_list=[]
for index in text_data ["availableCourses"]:
...
if os.path.isfile("parent.json"):
    with open("parent.json","r") as saral_data:
        text_data_1=json.load(saral_data)   
else:
    y=requests.get("http://saral.navgurukul.org/api/courses/"+str(text_data_1["availableCourses"][topic-1]["id"])+"/exercises")
    url2=requests.get(y)
    data=y.json()
    with open("parent.json","w") as saral_data:
        json.dump(data,saral_data,indent=4)
serial_no=1
serial_no1=1
topic_list=[]
for index1 in text_data_1["data"]:
...
if os.path.isfile("question.json"):
    with open("question.json","r") as saral_data:
        text_data_2=json.load(saral_data) 
else:  
    w=requests.get("http://saral.navgurukul.org/api/courses/"+str(text_data_2["availableCourses"][topic-1]["id"])+"/exercise/getBySlug?slug="+str(serial_no2["data"][slug-1]["childExercises"][que-1]["slug"]))
    DATA=w.json()
    with open("question.json","w") as saral_data:
        json.dump(DATA,saral_data,indent=4)
        print(DATA["content"])
